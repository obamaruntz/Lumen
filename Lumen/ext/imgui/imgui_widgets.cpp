...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        for (auto& o : offsets)
            window->DrawList->AddText(nullptr, 0, bb.Min + o, IM_COL32_BLACK, text_begin, text_end, wrap_width);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	const float w = GetContentRegionAvail().x; // full width

	// Render label above combo
	if (label_size.x > 0.0f)
	{
		RenderText(window->DC.CursorPos, label);
		window->DC.CursorPos.y += label_size.y + style.ItemInnerSpacing.y;
	}

	// Combo frame
	ImVec2 combo_pos = window->DC.CursorPos;
	ImRect bb(combo_pos, combo_pos + ImVec2(w, GetFrameHeight()));

	// Only account for combo in ItemSize to remove extra padding
	ItemSize(bb, 0.0f);
	if (!ItemAdd(bb, id))
		return false;
